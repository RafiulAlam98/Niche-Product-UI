...
const ManageProduct = ({ product }) => {
  ...
  const handleDelete = (id) => {
    ...
    if (proceed) {
      fetch(`https://niche-product-server-lemon.vercel.app/products/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
    ...
    <Col xs={12} md={12} lg={6}>
      <Card className="card-style mt-3 mb-5 ">
        ...
        <Card.Body className="d-flex flex-column justify-content-start align-items-start">
          <Card.Title className="fw-bold  card-text card-detail">
            {name}
          </Card.Title>
          <Card.Subtitle className=" fw-bold mb-3 mt-4 card-text">
            {color}
          </Card.Subtitle>
          <Card.Subtitle className="mb-3 card-text">
            {description}
          </Card.Subtitle>
